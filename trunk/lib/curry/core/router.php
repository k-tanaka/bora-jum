	...
	protected function _getParamsRewrite($basePath)
	{
		...
		if ($ini !== false) {
			foreach ($ini as $setting) {
				...
				if ($match) {
					if (array_key_exists('default_controller', $setting)) {
						$this->_controller = $setting['default_controller'];
					}
					if (array_key_exists('default_action', $setting)) {
						$this->_action = $setting['default_action'];
					}
					if (array_key_exists('controller', $setting)) {
						$controller = $setting['controller'];
					}
					if (array_key_exists('action', $setting)) {
						$action = $setting['action'];
					}
					if (array_key_exists('route', $setting)) {
						$this->_routingPath = trim($setting['route'], '/');
					}
					break;
				}
			}
